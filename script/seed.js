...
async function seed() {
  ...
  await Promise.all(
    zipData.features.map((zip) => {
      var filteredzip = zipSF.filter(
        (zip1) => zip1.RegionName == zip.properties.zip
      )[0];
  ...
  await Promise.all(
    state1B.map((state) => {
      let singleData = stateSinglePriceMed.filter(
        (single) => single.RegionName == state.RegionName
      );
      let twoBed = state2B.filter(
        (single) => single.RegionName == state.RegionName
      );
      let threeBed = state3B.filter(
        (single) => single.RegionName == state.RegionName
      );
      let fourBed = state4B.filter(
        (single) => single.RegionName == state.RegionName
      );
      let fiveBed = state5B.filter(
        (single) => single.RegionName == state.RegionName
      );
      let coopBed = stateCo.filter(
        (single) => single.RegionName == state.RegionName
      );
      let allBed = stateAH.filter(
        (single) => single.RegionName == state.RegionName
      );
  ...
  await Promise.all(
    county1B.map((state) => {
      let singleData = countySF.filter(
        (single) => single.RegionName == state.RegionName
      );
      let twoBed = county2B.filter(
        (single) => single.RegionName == state.RegionName
      );
      let threeBed = county3B.filter(
        (single) => single.RegionName == state.RegionName
      );
      let fourBed = county4B.filter(
        (single) => single.RegionName == state.RegionName
      );
      let fiveBed = county5B.filter(
        (single) => single.RegionName == state.RegionName
      );
      let coopBed = countyCo.filter(
        (single) => single.RegionName == state.RegionName
      );
      let allBed = countyAH.filter(
        (single) => single.RegionName == state.RegionName
      );
      return HistoricData.create({
        name: state.RegionName,
  ...
  await Promise.all(
    zip1B.map((state) => {
      let singleData = zipSF.filter(
        (single) => single.RegionName == state.RegionName
      );
      let twoBed = zip2B.filter(
        (single) => single.RegionName == state.RegionName
      );
      let threeBed = zip3B.filter(
        (single) => single.RegionName == state.RegionName
      );
      let fourBed = zip4B.filter(
        (single) => single.RegionName == state.RegionName
      );
      let fiveBed = zip5B.filter(
        (single) => single.RegionName == state.RegionName
      );
      let coopBed = zipCo.filter(
        (single) => single.RegionName == state.RegionName
      );
      let allBed = zipAH.filter(
        (single) => single.RegionName == state.RegionName
      );
      return HistoricData.create({
        name: state.RegionName,
